...
class TradingAgentsGraph:
    def __init__(self, selected_analysts=["market", "social", "news", "fundamentals"], debug=False, config: Dict[str, Any] = None):
        ...
        self.deep_thinking_llm = get_chat_model(self.config["deep_think_llm"], temperature=0.2)
        self.quick_thinking_llm = get_chat_model(self.config["quick_think_llm"], temperature=0.05)
    ...
    def propagate(self, company_name, trade_date, extra_input=None):
        self.ticker = company_name
        init_agent_state = self.propagator.create_initial_state(company_name, trade_date)
        if extra_input:
            init_agent_state["final_trade_decision"] = extra_input  # Inject summary
    ...
    def _log_state(self, trade_date, final_state):
        ...
        with open(directory / "full_states_log.json", "w") as f:
